...
async function POST({ request }) {
	...
	if (isNaN(course_id)) return new Response('Missing course ID', { status: 400 });

	// Create Link
	return await LinkHelper.create(course_id).then(
...
async function PUT({ request }) {
	...
	// Update Link
	return await LinkHelper.update(data).then(

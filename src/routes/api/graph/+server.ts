...
async function POST({ request }) {
	...
	if (isNaN(course_id)) return new Response('Missing course ID or name', { status: 400 });

	// Create graph
	return await GraphHelper.create(course_id, name).then(
...
async function PUT({ request }) {
	...
	// Update domain
	return await GraphHelper.update(data).then(

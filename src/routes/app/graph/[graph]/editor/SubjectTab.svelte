<script lang="ts">
	...
	import { SubjectController, SubjectRelationController } from '$scripts/controllers';
	...
	// Variables
	let filtered_subjects = $derived(
		$graph.subjects.filter((subject) => subject.matchesQuery($subject_query))
	);
	let filtered_relations = $derived(
		$graph.subject_relations.filter((relation) => relation.matchesQuery($subject_query))
	);
</script>
...
<div class="tab">
	...
	>
		<img src={plus_icon} alt="New subject" />
	</button>

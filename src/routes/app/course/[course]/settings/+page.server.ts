...
export const load: PageServerLoad = async ({ params }) => {
	...
	// Start data streams
	const course = CourseHelper.getById(
		course_id,
		'programs',
		'graphs',
		'links',
	...
	const programs = ProgramHelper.getAll().catch((error) => {
		throw new Error(error);
	});
	const users = UserHelper.getAll().catch((error) => {
		throw new Error(error);
	});
	const graphs = CourseHelper.getGraphs(course_id).catch((error) => {
		throw new Error(error);
	});
	const links = CourseHelper.getLinks(course_id).catch((error) => {
		throw new Error(error);
	});

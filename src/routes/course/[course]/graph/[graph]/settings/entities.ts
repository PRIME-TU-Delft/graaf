...
class Graph {
	...
	domainRelations(): DomainRelation[] {
		return this.relations.filter(relation => relation instanceof DomainRelation) as DomainRelation[]
	}

	subjectRelations(): SubjectRelation[] {
		return this.relations.filter(relation => relation instanceof SubjectRelation) as SubjectRelation[]
	}
		...
		if (this.child) {
			...
			if (!this.parent) {
				options = options.filter(domain =>
					!this.graph.relations.find(relation =>

<script lang="ts">
	...
	const { sandbox, graph, graphs, onSuccess = () => {} }: GraphLinksProps = $props();
...
<form action="?/edit-graph" method="POST" use:formEnhance>
		...
		<div class="my-2 grid grid-cols-1 gap-x-4 gap-y-2">
			{#each links as link (link.id)}
				<div in:fade class="flex w-full items-center justify-between gap-1">
					...
					{#if graphs.length > 1}
						<MoveAliasLink {sandbox} {graph} {graphs} {link} {onSuccess} />
					{/if}

					<DeleteAliasLink {sandbox} {graph} {link} {onSuccess} />
				</div>
			{/each}
		</div>

		<AddAliasLink {sandbox} {graph}
			onSuccess={(link) => {
				links.push(link);
			}}
		/>
	...
	<div class="flex items-center justify-between gap-1">
		...
		{#if hasSandboxPermissions(page.data.user, sandbox, 'OwnerOREditor')}
			<DeleteGraph {sandbox} {graph} {onSuccess} />
		{/if}

<script lang="ts">
	...
	async function handleChangeStyle(
		...
		const domain = graphValidator.graph.domains[domainIndex];
		domain.style = key;
	...
	function handleDndConsider(e: CustomEvent<{ items: (typeof graphValidator.graph)['domains'] }>) {
		graphValidator.graph.domains = e.detail.items;
	}
	async function handleDndFinalize(
		e: CustomEvent<{ items: (typeof graphValidator.graph)['domains'] }>
	) {
		graphValidator.graph.domains = e.detail.items;

		const body = graphValidator.graph.domains.map((domain, index) => ({
			domainId: domain.id,
			newOrder: index
		}));
		...
		if (!response.ok) {
			...
			graphValidator.graph.domains = graphValidator.graph.domains.toSorted(
				(a, b) => a.order - b.order
			);
			...
			graphValidator.graph.domains.forEach((domain, index) => {
				domain.order = index;
			});
		}
	}
...
<Grid.Root columnTemplate={['3rem', '3rem', 'minmax(12rem, 1fr)', '5rem', '5rem']}>
	...
	<Grid.ReorderRows
		name="domain"
		items={graphValidator.graph.domains}
		onconsider={handleDndConsider}
		onfinalize={handleDndFinalize}
	>
		{#snippet children(domain, index)}
			<Grid.Cell>
				{@const issues = graphValidator.issues.domainIssues[domain.id] || []}
			...
			<Grid.Cell>
				<ChangeDomain graph={graphValidator.graph} {domain} />
			</Grid.Cell>
		{/snippet}
	</Grid.ReorderRows>
</Grid.Root>
	...
	<Popover.Root bind:open={isOpenState.isOpen}>
		...
		<Popover.Content side="right" class="space-y-1">
			...
			<p class="pb-4 text-xs text-gray-700">
				For domain: {graphValidator.graph.domains[domainIndex].name}
			</p>
			<Button
				variant="outline"
				class={cn(

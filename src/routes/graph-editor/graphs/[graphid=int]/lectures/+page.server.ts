...
export const load: ServerLoad = async () => {
	return {
		newLectureForm: await superValidate(valibot(lectureSchema)),
		deleteLectureForm: await superValidate(valibot(deleteLectureSchema))
	};
};

export const actions = {
	'add-lecture-to-graph': async (event) => {
		const form = await superValidate(event, valibot(lectureSchema));
	...
	'delete-lecture': async (event) => {
		const form = await superValidate(event, valibot(deleteLectureSchema));

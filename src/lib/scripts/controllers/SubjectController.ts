...
import type { DomainStyle, DropdownOption, SerializedSubject } from '$scripts/types';
...
class SubjectController extends NodeController<SubjectController> {
	...
	get parents(): SubjectController[] {
		if (this._parent_ids === undefined) throw new Error('SubjectError: Parent data unknown');
		if (this._parents !== undefined) return Array.from(this._parents);
	...
	get children(): SubjectController[] {
		if (this._child_ids === undefined) throw new Error('SubjectError: Child data unknown');
		if (this._children !== undefined) return Array.from(this._children);
	...
	get domain_id(): number | null {
		if (this._domain_id === undefined) throw new Error('SubjectError: Domain data unknown');
		return this._domain_id;
	}

	get domain(): DomainController | null {
		if (this._domain_id === undefined) throw new Error('SubjectError: Domain data unknown');
		if (this._domain !== undefined) return this._domain;

		// Fetch domain from cache
		this._domain = this._domain_id
			? this.cache.findOrThrow(DomainController, this._domain_id)
			: null;
		return this._domain;
	}
	...
	get lecture_ids(): number[] {
		if (this._lecture_ids === undefined) throw new Error('SubjectError: Lecture data unknown');
	...
	assignParent(parent: SubjectController, mirror: boolean = true) {
		if (this._parent_ids !== undefined) {
			if (this._parent_ids.includes(parent.id))
				throw new Error(
					`SubjectError: Parent with ID ${parent.id} already assigned to subject with ID ${this.id}`
				);
	...
	assignChild(child: SubjectController, mirror: boolean = true): void {
		if (this._child_ids !== undefined) {
			if (this._child_ids.includes(child.id))
				throw new Error(
					`SubjectError: Child with ID ${child.id} already assigned to subject with ID ${this.id}`
				);
	...
	assignDomain(domain: DomainController, mirror: boolean = true): void {
		if (this._domain_id !== undefined) {
			if (this._domain_id === domain.id)
				throw new Error(
					`SubjectError: Domain with ID ${domain.id} already assigned to subject with ID ${this.id}`
				);
			if (this._domain_id !== null && mirror) this.domain?.unassignSubject(this, false);
	...
	assignToLecture(lecture: LectureController, mirror: boolean = true): void {
		if (this._lecture_ids !== undefined) {
			if (this._lecture_ids.includes(lecture.id))
				throw new Error(
					`SubjectError: Lecture with ID ${lecture.id} already assigned to subject with ID ${this.id}`
				);
	...
	unassignParent(parent: SubjectController, mirror: boolean = true): void {
		if (this._parent_ids !== undefined) {
			if (!this._parent_ids.includes(parent.id))
				throw new Error(
					`SubjectError: Parent with ID ${parent.id} not assigned to subject with ID ${this.id}`
				);
	...
	unassignChild(child: SubjectController, mirror: boolean = true): void {
		if (this._child_ids !== undefined) {
			if (!this._child_ids.includes(child.id))
				throw new Error(
					`SubjectError: Child with ID ${child.id} not assigned to subject with ID ${this.id}`
				);
	...
	unassignFromLecture(lecture: LectureController, mirror: boolean = true): void {
		if (this._lecture_ids !== undefined) {
			if (!this._lecture_ids.includes(lecture.id))
				throw new Error(
					`SubjectError: Lecture with ID ${lecture.id} not assigned to subject with ID ${this.id}`
				);
	...
	static revive(cache: ControllerCache, data: SerializedSubject): SubjectController {
		...
		if (subject !== undefined) {
			...
			if (!subject.represents(data)) {
				throw new Error(
					`SubjectError: Subject with ID ${data.id} already exists, and is inconsistent with new data`
				);
			}

			// Update subject where necessary
			if (subject._graph_id === undefined) subject._graph_id = data.graph_id;
			if (subject._parent_ids === undefined) subject._parent_ids = data.parent_ids;
			if (subject._child_ids === undefined) subject._child_ids = data.child_ids;
			if (subject._lecture_ids === undefined) subject._lecture_ids = data.lecture_ids;
	...
	represents(data: SerializedSubject): boolean {
		return (
			...
			this.x === data.x &&
			this.y === data.y &&
			(this._domain_id === undefined ||
				data.domain_id === undefined ||
				this._domain_id === data.domain_id) &&
			(this._graph_id === undefined ||
				data.graph_id === undefined ||
				this._graph_id === data.graph_id) &&
			(this._parent_ids === undefined ||
				data.parent_ids === undefined ||
				compareArrays(this._parent_ids, data.parent_ids)) &&
			(this._child_ids === undefined ||
				data.child_ids === undefined ||
				compareArrays(this._child_ids, data.child_ids)) &&
	...
	async delete() {
		...
		if (this._graph_id !== undefined) this.graph.unassignSubject(this);
		if (this._domain_id !== undefined) this.domain?.unassignSubject(this, false);
		if (this._parent_ids !== undefined)
			for (const parent of this.parents) parent.unassignChild(this, false);
		if (this._child_ids !== undefined)
			for (const child of this.children) child.unassignParent(this, false);
		if (this._lecture_ids !== undefined)
			for (const lecture of this.lectures) lecture.unassignSubject(this, false);
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/subject/${this.id} DELETE): ${response.status} ${response.statusText}`
			);
		}

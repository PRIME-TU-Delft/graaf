...
class LinkController {
	...
	get course_id(): number {
		if (this._course_id === undefined) throw new Error('LinkError: Course data unknown');
	...
	get graph_id(): number | null {
		if (this._graph_id === undefined) throw new Error('LinkError: Graph data unknown');
	...
	assignGraph(graph: GraphController, mirror: boolean = true): void {
		if (this._graph_id !== undefined) {
			if (this._graph_id === graph.id)
				throw new Error(
					`LinkError: Graph with ID ${graph.id} already assigned to link with ID ${this.id}`
				);
			if (this._graph_id !== null && mirror) this.graph?.unassignFromLink(this, false);
	...
	unassignGraph(graph: GraphController, mirror: boolean = true): void {
		if (this._graph_id !== undefined) {
			if (this._graph_id !== graph.id)
				throw new Error(
					`LinkError: Graph with ID ${graph.id} not assigned to link with ID ${this.id}`
				);
			this._graph_id = null;
			this._graph = null;
			this._unchanged = false;
	...
	validateName(strict: boolean = true): Validation {
		...
		if (this.trimmed_name === '') {
			...
			this.course.links.find((link) => link !== this && link.trimmed_name === this.trimmed_name)
		) {
			validation.add({
				severity: Severity.error,
	...
	static revive(cache: ControllerCache, data: SerializedLink): LinkController {
		...
		if (link !== undefined) {
			...
			if (!link.represents(data)) {
				throw new Error(
					`LinkError: Link with ID ${data.id} already exists, and is inconsistent with new data`
				);
			}

			// Update link where necessary
			if (link._course_id === undefined) link._course_id = data.course_id;
			if (link._graph_id === undefined) link._graph_id = data.graph_id;
	...
	async delete() {
		...
		if (this._course_id !== undefined) this.course.unassignLink(this);
		if (this._graph_id !== undefined) this.graph?.unassignFromLink(this, false);
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/link/${this.id} DELETE): ${response.status} ${response.statusText}`
			);
		}

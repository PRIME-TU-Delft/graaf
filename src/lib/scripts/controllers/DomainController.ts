...
import type { DomainStyle, DropdownOption, SerializedDomain } from '$scripts/types';
...
class DomainController extends NodeController<DomainController> {
	...
	get parents(): DomainController[] {
		if (this._parent_ids === undefined) throw new Error('DomainError: Parent data unknown');
	...
	get children(): DomainController[] {
		if (this._child_ids === undefined) throw new Error('DomainError: Child data unknown');
	...
	get subject_ids(): number[] {
		if (this._subject_ids === undefined) throw new Error('DomainError: Subject data unknown');
	...
	assignParent(parent: DomainController, mirror: boolean = true): void {
		if (this._parent_ids !== undefined) {
			if (this._parent_ids.includes(parent.id))
				throw new Error(
					`DomainError: Parent with ID ${parent.id} already assigned to domain with ID ${this.id}`
				);
	...
	assignChild(child: DomainController, mirror: boolean = true): void {
		if (this._child_ids !== undefined) {
			if (this._child_ids.includes(child.id))
				throw new Error(
					`DomainError: Child with ID ${child.id} already assigned to domain with ID ${this.id}`
				);
	...
	assignSubject(subject: SubjectController, mirror: boolean = true): void {
		if (this._subject_ids !== undefined) {
			if (this._subject_ids.includes(subject.id))
				throw new Error(
					`DomainError: Subject with ID ${subject.id} already assigned to domain with ID ${this.id}`
				);
	...
	unassignParent(parent: DomainController, mirror: boolean = true): void {
		if (this._parent_ids !== undefined) {
			if (!this._parent_ids.includes(parent.id))
				throw new Error(
					`DomainError: Parent with ID ${parent.id} not assigned to domain with ID ${this.id}`
				);
	...
	unassignChild(child: DomainController, mirror: boolean = true): void {
		if (this._child_ids !== undefined) {
			if (!this._child_ids.includes(child.id))
				throw new Error(
					`DomainError: Child with ID ${child.id} not assigned to domain with ID ${this.id}`
				);
	...
	unassignSubject(subject: SubjectController, mirror: boolean = true): void {
		if (this._subject_ids !== undefined) {
			if (!this._subject_ids.includes(subject.id))
				throw new Error(
					`DomainError: Subject with ID ${subject.id} not assigned to domain with ID ${this.id}`
				);
	...
	static revive(cache: ControllerCache, data: SerializedDomain): DomainController {
		...
		if (domain !== undefined) {
			...
			if (!domain.represents(data)) {
				throw new Error(
					`DomainError: Domain with ID ${data.id} already exists, and is inconsistent with new data`
				);
			}

			// Update domain where necessary
			if (domain._graph_id === undefined) domain._graph_id = data.graph_id;
			if (domain._parent_ids === undefined) domain._parent_ids = data.parent_ids;
			if (domain._child_ids === undefined) domain._child_ids = data.child_ids;
			if (domain._subject_ids === undefined) domain._subject_ids = data.subject_ids;
	...
	async delete(reorder_graph: boolean = true) {
		...
		if (this._graph_id !== undefined) this.graph.unassignDomain(this);
		if (this._subject_ids !== undefined)
			for (const subject of this.subjects) subject.unassignDomain(this, false);
		for (const relation of this.graph.domain_relations) {
			if (relation.parent === this) relation.parent = null;
			else if (relation.child === this) relation.child = null;
		}
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/domain/${this.id} DELETE): ${response.status} ${response.statusText}`
			);
		}
	...
	matchesQuery(query: string): boolean {
		...
		const lower_style = this.style
			? settings.NODE_STYLES[this.style].display_name.toLowerCase()
			: '';

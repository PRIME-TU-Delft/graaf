...
import { ControllerCache, ProgramController, CourseController } from '$scripts/controllers';
...
class UserController {
	...
	get course_editors(): CourseController[] {
		if (this._course_editors !== undefined) return Array.from(this._course_editors);
		this._course_editors = this.course_editor_ids.map((id) =>
			this.cache.findOrThrow(CourseController, id)
		);
	...
	get course_admins(): CourseController[] {
		if (this._course_admins !== undefined) return Array.from(this._course_admins);
		this._course_admins = this.course_admin_ids.map((id) =>
			this.cache.findOrThrow(CourseController, id)
		);
	...
	get program_editors(): ProgramController[] {
		if (this._program_editors !== undefined) return Array.from(this._program_editors);
		this._program_editors = this.program_editor_ids.map((id) =>
			this.cache.findOrThrow(ProgramController, id)
		);
	...
	get program_admins(): ProgramController[] {
		if (this._program_admins !== undefined) return Array.from(this._program_admins);
		this._program_admins = this.program_admin_ids.map((id) =>
			this.cache.findOrThrow(ProgramController, id)
		);
	...
	becomeCourseEditor(course: CourseController, mirror: boolean = true) {
		if (this._course_editor_ids !== undefined) {
			if (this._course_editor_ids.includes(course.id))
				throw new Error(
					`UserError: Course editor with ID ${course.id} already assigned to user with ID ${this.id}`
				);
	...
	becomeCourseAdmin(course: CourseController, mirror: boolean = true) {
		if (this._course_admin_ids !== undefined) {
			if (this._course_admin_ids.includes(course.id))
				throw new Error(
					`UserError: Course admin with ID ${course.id} already assigned to user with ID ${this.id}`
				);
	...
	becomeProgramEditor(program: ProgramController, mirror: boolean = true) {
		if (this._program_editor_ids !== undefined) {
			if (this._program_editor_ids.includes(program.id))
				throw new Error(
					`UserError: Program editor with ID ${program.id} already assigned to user with ID ${this.id}`
				);
	...
	becomeProgramAdmin(program: ProgramController, mirror: boolean = true) {
		if (this._program_admin_ids !== undefined) {
			if (this._program_admin_ids.includes(program.id))
				throw new Error(
					`UserError: Program admin with ID ${program.id} already assigned to user with ID ${this.id}`
				);
	...
	resignAsCourseEditor(course: CourseController, mirror: boolean = true) {
		if (this._course_editor_ids !== undefined) {
			if (!this._course_editor_ids.includes(course.id))
				throw new Error(
					`UserError: Course editor with ID ${course.id} not assigned to user with ID ${this.id}`
				);
	...
	resignAsCourseAdmin(course: CourseController, mirror: boolean = true) {
		if (this._course_admin_ids !== undefined) {
			if (!this._course_admin_ids.includes(course.id))
				throw new Error(
					`UserError: Course admin with ID ${course.id} not assigned to user with ID ${this.id}`
				);
	...
	resignAsProgramEditor(program: ProgramController, mirror: boolean = true) {
		if (this._program_editor_ids !== undefined) {
			if (!this._program_editor_ids.includes(program.id))
				throw new Error(
					`UserError: Program editor with ID ${program.id} not assigned to user with ID ${this.id}`
				);
	...
	resignAsProgramAdmin(program: ProgramController, mirror: boolean = true) {
		if (this._program_admin_ids !== undefined) {
			if (!this._program_admin_ids.includes(program.id))
				throw new Error(
					`UserError: Program admin with ID ${program.id} not assigned to user with ID ${this.id}`
				);
	...
	static revive(cache: ControllerCache, data: SerializedUser): UserController {
		...
		if (user !== undefined) {
			...
			if (!user.represents(data)) {
				throw new Error(
					`UserError: User with ID ${data.id} already exists, and is inconsistent with new data`
				);
			}

			// Update user where necessary
			if (user._course_editor_ids === undefined) user._course_editor_ids = data.course_editor_ids;
			if (user._course_admin_ids === undefined) user._course_admin_ids = data.course_admin_ids;
			if (user._program_editor_ids === undefined)
				user._program_editor_ids = data.program_editor_ids;
			if (user._program_admin_ids === undefined) user._program_admin_ids = data.program_admin_ids;
	...
	represents(data: SerializedUser): boolean {
		return (
			...
			(this._course_editor_ids === undefined ||
				data.course_editor_ids === undefined ||
				compareArrays(this._course_editor_ids, data.course_editor_ids)) &&
			(this._course_admin_ids === undefined ||
				data.course_admin_ids === undefined ||
				compareArrays(this._course_admin_ids, data.course_admin_ids)) &&
			(this._program_editor_ids === undefined ||
				data.program_editor_ids === undefined ||
				compareArrays(this._program_editor_ids, data.program_editor_ids)) &&
			(this._program_admin_ids === undefined ||
				data.program_admin_ids === undefined ||
				compareArrays(this._program_admin_ids, data.program_admin_ids))
		);
	}

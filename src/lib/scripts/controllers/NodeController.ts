...
abstract class NodeController<T extends DomainController | SubjectController> {
	...
	// Graph properties
	get graph_id(): number {
		if (this._graph_id === undefined) throw new Error('NodeError: Graph data unknown');
	...
	get parent_ids(): number[] {
		if (this._parent_ids === undefined) throw new Error('NodeError: Parent data unknown');
		return Array.from(this._parent_ids);
	}

	// Child properties
	get child_ids(): number[] {
		if (this._child_ids === undefined) throw new Error('NodeError: Child data unknown');

...
import { ControllerCache, CourseController, UserController } from '$scripts/controllers';
...
class ProgramController {
	...
	get course_ids(): number[] {
		if (this._course_ids === undefined) throw new Error('ProgramError: Course data unknown');
		return Array.from(this._course_ids);
	}

	get courses(): CourseController[] {
		if (this._course_ids === undefined) throw new Error('ProgramError: Course data unknown');
		if (this._courses !== undefined) return Array.from(this._courses);
	...
	get editor_ids(): string[] {
		if (this._editor_ids === undefined) throw new Error('ProgramError: Editor data unknown');
	...
	get admin_ids(): string[] {
		if (this._admin_ids === undefined) throw new Error('ProgramError: Admin data unknown');
	...
	assignCourse(course: CourseController, mirror: boolean = true) {
		if (this._course_ids !== undefined) {
			if (this._course_ids.includes(course.id))
				throw new Error(
					`ProgramError: Course with ID ${course.id} already assigned to program with ID ${this.id}`
				);
	...
	assignEditor(editor: UserController, mirror: boolean = true) {
		if (this._editor_ids !== undefined) {
			if (this._editor_ids.includes(editor.id))
				throw new Error(
					`ProgramError: Editor with ID ${editor.id} already assigned to program with ID ${this.id}`
				);
	...
	assignAdmin(admin: UserController, mirror: boolean = true) {
		if (this._admin_ids !== undefined) {
			if (this._admin_ids.includes(admin.id))
				throw new Error(
					`ProgramError: Admin with ID ${admin.id} already assigned to program with ID ${this.id}`
				);
	...
	unassignCourse(course: CourseController, mirror: boolean = true) {
		if (this._course_ids !== undefined) {
			if (!this._course_ids.includes(course.id))
				throw new Error(
					`ProgramError: Course with ID ${course.id} not assigned to program with ID ${this.id}`
				);
	...
	unassignEditor(editor: UserController, mirror: boolean = true) {
		if (this._editor_ids !== undefined) {
			if (!this._editor_ids.includes(editor.id))
				throw new Error(
					`ProgramError: Editor with ID ${editor.id} not assigned to program with ID ${this.id}`
				);
	...
	unassignAdmin(admin: UserController, mirror: boolean = true) {
		if (this._admin_ids === undefined) return;
		if (!this._admin_ids.includes(admin.id))
			throw new Error(
				`ProgramError: Admin with ID ${admin.id} not assigned to program with ID ${this.id}`
			);
	...
	static revive(cache: ControllerCache, data: SerializedProgram): ProgramController {
		...
		if (program !== undefined) {
			...
			if (!program.represents(data)) {
				throw new Error(
					`ProgramError: Program with ID ${data.id} already exists, and is inconsistent with new data`
				);
			}

			// Update the existing program where necessary
			if (program._course_ids === undefined) program._course_ids = data.course_ids;
			if (program._editor_ids === undefined) program._editor_ids = data.editor_ids;
			if (program._admin_ids === undefined) program._admin_ids = data.admin_ids;
	...
	represents(data: SerializedProgram): boolean {
		return (
			...
			(this._course_ids === undefined ||
				data.course_ids === undefined ||
				compareArrays(this._course_ids, data.course_ids)) &&
	...
	async delete() {
		...
		if (this._course_ids !== undefined)
			for (const course of this.courses) course.unassignFromProgram(this, false);
		if (this._editor_ids !== undefined)
			for (const editor of this.editors) editor.resignAsProgramEditor(this, false);
		if (this._admin_ids !== undefined)
			for (const admin of this.admins) admin.resignAsProgramAdmin(this, false);
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/program/${this.id} DELETE): ${response.status} ${response.statusText}`
			);
		}

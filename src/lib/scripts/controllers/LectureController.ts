...
import type { DropdownOption, SerializedLecture } from '$scripts/types';
...
class LectureController {
	...
	// Graph properties
	get graph_id(): number {
		if (this._graph_id === undefined) throw new Error('LectureError: Graph data unknown');
	...
	get present_subjects(): SubjectController[] {
		if (this._present_subject_ids === undefined)
			throw new Error('LectureError: Subject data unknown');
		if (this._present_subjects !== undefined) return Array.from(this._present_subjects);

		// Fetch subjects from cache
		this._present_subjects = this._present_subject_ids.map((id) =>
			this.cache.findOrThrow(SubjectController, id)
		);
	...
	get future_subjects(): SubjectController[] {
		...
		for (const subject of this.present_subjects) {
			for (const child of subject.children) {
				if (!result.includes(child) && !this.present_subjects.includes(child)) result.push(child);
			}
		}
	...
	get subjects(): SubjectController[] {
		return this.present_subjects.concat(this.past_subjects).concat(this.future_subjects);
	}
	...
	get relations(): SubjectRelationController[] {
		return this.graph.subject_relations
			...
			.filter(
				(relation) =>
					(this.past_subjects.includes(relation.parent!) &&
						this.present_subjects.includes(relation.child!)) ||
					(this.present_subjects.includes(relation.parent!) &&
						this.future_subjects.includes(relation.child!))
			);
	}
	...
	get max_height(): number {
		return Math.max(
			this.present_subjects.length,
			this.past_subjects.length,
			this.future_subjects.length
		);
	}
	...
	assignSubject(subject: SubjectController, mirror: boolean = true) {
		if (this._present_subject_ids !== undefined) {
			if (this._present_subject_ids.includes(subject.id))
				throw new Error(
					`LectureError: Subject with ID ${subject.id} already assigned to lecture with ID ${this.id}`
				);
	...
	unassignSubject(subject: SubjectController, mirror: boolean = true) {
		if (this._present_subject_ids !== undefined) {
			if (!this._present_subject_ids.includes(subject.id))
				throw new Error(
					`LectureError: Subject with ID ${subject.id} not assigned to lecture with ID ${this.id}`
				);
	...
	private otherLectureWithDuplicateName(): LectureController | undefined {
		return this.graph.lectures.find(
			(lecture) => lecture.id !== this.id && lecture.trimmed_name === this.trimmed_name
		);
	}
	...
	private otherLecturesCoveringSubject(subject: SubjectController): LectureController[] {
		return this.graph.lectures.filter(
			(lecture) => lecture.id !== this.id && lecture.present_subject_ids.includes(subject.id)
		);
	}
	...
	static revive(cache: ControllerCache, data: SerializedLecture): LectureController {
		...
		if (lecture !== undefined) {
			...
			if (!lecture.represents(data)) {
				throw new Error(
					`LectureError: Lecture with ID ${data.id} already exists, and is inconsistent with new data`
				);
			}

			// Update lecture where necessary
			if (lecture._graph_id === undefined) lecture._graph_id = data.graph_id;
	...
	represents(data: SerializedLecture): boolean {
		return (
			...
			(this._present_subject_ids === undefined ||
				data.subject_ids === undefined ||
				compareArrays(this._present_subject_ids, data.subject_ids))
		);
	}
	...
	async delete(reorder_graph: boolean = true) {
		...
		if (this._graph_id !== undefined) this.graph.unassignLecture(this);
		if (this._present_subject_ids !== undefined)
			for (const subject of this.present_subjects) subject.unassignFromLecture(this, false);
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/lecture/${this.id} DELETE): ${response.status} ${response.statusText}`
			);
		}
	...
	matchesQuery(query: string): boolean {
		...
		const lower_subjects = this.present_subjects.map((subject) =>
			subject.trimmed_name.toLowerCase()
		);

		return (
			lower_name.includes(lower_query) ||
			lower_subjects.some((subject) => subject.includes(lower_query))
		);
	}
}

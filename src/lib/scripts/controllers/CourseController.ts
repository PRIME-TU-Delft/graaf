...
class CourseController {
	...
	get program_ids(): number[] {
		if (this._program_ids === undefined) throw new Error('CourseError: Program data unknown');
		return Array.from(this._program_ids);
	}

	get programs(): ProgramController[] {
		if (this._program_ids === undefined) throw new Error('CourseError: Program data unknown');
		if (this._programs !== undefined) return Array.from(this._programs);
	...
	get graph_ids(): number[] {
		if (this._graph_ids === undefined) throw new Error('CourseError: Graph data unknown');
		return Array.from(this._graph_ids);
	}

	get graphs(): GraphController[] {
		if (this._graph_ids === undefined) throw new Error('CourseError: Graph data unknown');
		if (this._graphs !== undefined) return Array.from(this._graphs);
	...
	// Link properties
	get link_ids(): number[] {
		if (this._link_ids === undefined) throw new Error('CourseError: Link data unknown');
	...
	get editor_ids(): string[] {
		if (this._editor_ids === undefined) throw new Error('CourseError: Editor data unknown');
		return Array.from(this._editor_ids);
	}

	get editors(): UserController[] {
		if (this._editor_ids === undefined) throw new Error('CourseError: Editor data unknown');
		if (this._editors !== undefined) return Array.from(this._editors);
	...
	get admin_ids(): string[] {
		if (this._admin_ids === undefined) throw new Error('CourseError: Admin data unknown');
		return Array.from(this._admin_ids);
	}

	get admins(): UserController[] {
		if (this._admin_ids === undefined) throw new Error('CourseError: Admin data unknown');
		if (this._admins !== undefined) return Array.from(this._admins);
	...
	assignToProgram(program: ProgramController, mirror: boolean = true) {
		if (this._program_ids !== undefined) {
			if (this._program_ids.includes(program.id))
				throw new Error(
					`CourseError: Program with ID ${program.id} already assigned to course with ID ${this.id}`
				);
	...
	assignGraph(graph: GraphController) {
		if (this._graph_ids !== undefined) {
			if (this._graph_ids.includes(graph.id))
				throw new Error(
					`CourseError: Graph with ID ${graph.id} already assigned to course with ID ${this.id}`
				);
	...
	assignLink(link: LinkController) {
		if (this._link_ids !== undefined) {
			if (this._link_ids.includes(link.id))
				throw new Error(
					`CourseError: Link with ID ${link.id} already assigned to course with ID ${this.id}`
				);
	...
	assignEditor(editor: UserController, mirror: boolean = true) {
		if (this._editor_ids !== undefined) {
			if (this._editor_ids.includes(editor.id))
				throw new Error(
					`CourseError: Editor with ID ${editor.id} already assigned to course with ID ${this.id}`
				);
	...
	assignAdmin(admin: UserController, mirror: boolean = true) {
		if (this._admin_ids !== undefined) {
			if (this._admin_ids.includes(admin.id))
				throw new Error(
					`CourseError: Admin with ID ${admin.id} already assigned to course with ID ${this.id}`
				);
	...
	unassignFromProgram(program: ProgramController, mirror: boolean = true) {
		if (this._program_ids !== undefined) {
			if (!this._program_ids.includes(program.id))
				throw new Error(
					`CourseError: Program with ID ${program.id} not assigned to course with ID ${this.id}`
				);
	...
	unassignGraph(graph: GraphController) {
		if (this._graph_ids !== undefined) {
			if (!this._graph_ids.includes(graph.id))
				throw new Error(
					`CourseError: Graph with ID ${graph.id} not assigned to course with ID ${this.id}`
				);
	...
	unassignLink(link: LinkController) {
		if (this._link_ids !== undefined) {
			if (!this._link_ids.includes(link.id))
				throw new Error(
					`CourseError: Link with ID ${link.id} not assigned to course with ID ${this.id}`
				);
	...
	unassignEditor(editor: UserController, mirror: boolean = true) {
		if (this._editor_ids !== undefined) {
			if (!this._editor_ids.includes(editor.id))
				throw new Error(
					`CourseError: Editor with ID ${editor.id} not assigned to course with ID ${this.id}`
				);
	...
	unassignAdmin(admin: UserController, mirror: boolean = true) {
		if (this._admin_ids !== undefined) {
			if (!this._admin_ids.includes(admin.id))
				throw new Error(
					`CourseError: Admin with ID ${admin.id} not assigned to course with ID ${this.id}`
				);
	...
	static revive(cache: ControllerCache, data: SerializedCourse): CourseController {
		...
		if (course !== undefined) {
			...
			if (!course.represents(data)) {
				throw new Error(
					`CourseError: Course with ID ${data.id} already exists, and is inconsistent with new data`
				);
			}

			// Update the existing course where necessary
			if (course._program_ids === undefined) course._program_ids = data.program_ids;
			if (course._graph_ids === undefined) course._graph_ids = data.graph_ids;
			if (course._link_ids === undefined) course._link_ids = data.link_ids;
			if (course._editor_ids === undefined) course._editor_ids = data.editor_ids;
			if (course._admin_ids === undefined) course._admin_ids = data.admin_ids;
	...
	represents(data: SerializedCourse): boolean {
		return (
			...
			(this._program_ids === undefined ||
				data.program_ids === undefined ||
				compareArrays(this._program_ids, data.program_ids)) &&
			(this._graph_ids === undefined ||
				data.graph_ids === undefined ||
				compareArrays(this._graph_ids, data.graph_ids)) &&
			(this._link_ids === undefined ||
				data.link_ids === undefined ||
				compareArrays(this._link_ids, data.link_ids)) &&
			(this._editor_ids === undefined ||
				data.editor_ids === undefined ||
				compareArrays(this._editor_ids, data.editor_ids)) &&
			(this._admin_ids === undefined ||
				data.admin_ids === undefined ||
				compareArrays(this._admin_ids, data.admin_ids))
		);
	}
	...
	async delete() {
		...
		if (this._program_ids !== undefined)
			for (const program of this.programs) program.unassignCourse(this, false);
		if (this._editor_ids !== undefined)
			for (const editor of this.editors) editor.resignAsCourseEditor(this, false);
		if (this._admin_ids !== undefined)
			for (const admin of this.admins) admin.resignAsCourseAdmin(this, false);
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/course/${this.id} DELETE): ${response.status} ${response.statusText}`
			);
		}

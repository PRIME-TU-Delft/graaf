...
abstract class RelationController<T extends DomainController | SubjectController> {
	...
	constructor(public graph: GraphController) {}
	...
	async save(save_status?: SaveStatus) {
		...
		await Promise.all([this.parent?.save(), this.child?.save()]);
...
class DomainRelationController extends RelationController<DomainController> {
	...
	protected isDuplicate(parent: DomainController, child: DomainController): boolean {
		...
		return (
			relations.find(
				(relation) =>
					relation.uuid !== this.uuid && relation.parent === parent && relation.child === child
			) !== undefined
		);
	}
	...
	protected isInconsistent(parent: DomainController, child: DomainController): boolean {
		...
		while (stack.length > 0) {
			...
			if (visited.includes(subject)) continue;
			if (subject.domain_id === child.id) return false;
	...
	static create(
		...
		parent: DomainController | null = null,
		child: DomainController | null = null
...
class SubjectRelationController extends RelationController<SubjectController> {
	...
	protected isInconsistent(parent: SubjectController, child: SubjectController): boolean {
		...
		while (stack.length > 0) {
			...
			if (visited.includes(domain)) continue;
			if (domain.id === child.domain_id) return false;
	...
	static create(
		graph: GraphController,
		parent: SubjectController | null = null,
		child: SubjectController | null = null

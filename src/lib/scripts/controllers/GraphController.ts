...
class GraphController {
	...
	get course_id(): number {
		if (this._course_id === undefined) throw new Error('GraphError: Course data unknown');
		return this._course_id;
	}

	get course(): CourseController {
		if (this._course_id === undefined) throw new Error('GraphError: Course data unknown');
		if (this._course !== undefined) return this._course;
	...
	get domain_ids(): number[] {
		if (this._domain_ids === undefined) throw new Error('GraphError: Domain data unknown');
		return Array.from(this._domain_ids);
	}

	get domains(): DomainController[] {
		if (this._domain_ids === undefined) throw new Error('GraphError: Domain data unknown');
		if (this._domains !== undefined) return Array.from(this._domains);
	...
	get domain_relations(): DomainRelationController[] {
		if (this._domains_relations !== undefined) return Array.from(this._domains_relations);
	...
	get subject_ids(): number[] {
		if (this._subject_ids === undefined) throw new Error('GraphError: Subject data unknown');
		return Array.from(this._subject_ids);
	}

	get subjects(): SubjectController[] {
		if (this._subject_ids === undefined) throw new Error('GraphError: Subject data unknown');
		if (this._subjects !== undefined) return Array.from(this._subjects);
	...
	get subject_relations(): SubjectRelationController[] {
		if (this._subjects_relations !== undefined) return Array.from(this._subjects_relations);
	...
	get lecture_ids(): number[] {
		if (this._lecture_ids === undefined) throw new Error('GraphError: Lecture data unknown');
		return Array.from(this._lecture_ids);
	}

	get lectures(): LectureController[] {
		if (this._lecture_ids === undefined) throw new Error('GraphError: Lecture data unknown');
		if (this._lectures !== undefined) return Array.from(this._lectures);
	...
	// Link properties
	get link_ids(): number[] {
		if (this._link_ids === undefined) throw new Error('GraphError: Link data unknown');
		return Array.from(this._link_ids);
	}

	get links(): LinkController[] {
		if (this._link_ids === undefined) throw new Error('GraphError: Link data unknown');
		if (this._links !== undefined) return Array.from(this._links);
	...
	assignDomain(domain: DomainController) {
		if (this._domain_ids !== undefined) {
			if (this._domain_ids.includes(domain.id))
				throw new Error(
					`GraphError: Domain with ID ${domain.id} already assigned to graph with ID ${this.id}`
				);
	...
	assignSubject(subject: SubjectController) {
		if (this._subject_ids !== undefined) {
			if (this._subject_ids.includes(subject.id))
				throw new Error(
					`GraphError: Subject with ID ${subject.id} already assigned to graph with ID ${this.id}`
				);
	...
	assignSubjectRelation(relation: SubjectRelationController) {
		if (this._subjects_relations !== undefined) {
			if (this._subjects_relations.includes(relation))
				throw new Error(
					`GraphError: Subject relation already assigned to graph with ID ${this.id}`
				);
	...
	assignLecture(lecture: LectureController) {
		if (this._lecture_ids !== undefined) {
			if (this._lecture_ids.includes(lecture.id))
				throw new Error(
					`GraphError: Lecture with ID ${lecture.id} already assigned to graph with ID ${this.id}`
				);
	...
	assignToLink(link: LinkController, mirror: boolean = true) {
		if (this._link_ids !== undefined) {
			if (this._link_ids.includes(link.id))
				throw new Error(
					`GraphError: Link with ID ${link.id} already assigned to graph with ID ${this.id}`
				);
	...
	unassignDomain(domain: DomainController) {
		if (this._domain_ids !== undefined) {
			if (!this._domain_ids.includes(domain.id))
				throw new Error(
					`GraphError: Domain with ID ${domain.id} not assigned to graph with ID ${this.id}`
				);
	...
	unassignSubject(subject: SubjectController) {
		if (this._subject_ids !== undefined) {
			if (!this._subject_ids.includes(subject.id))
				throw new Error(
					`GraphError: Subject with ID ${subject.id} not assigned to graph with ID ${this.id}`
				);
	...
	unassignLecture(lecture: LectureController) {
		if (this._lecture_ids !== undefined) {
			if (!this._lecture_ids.includes(lecture.id))
				throw new Error(
					`GraphError: Lecture with ID ${lecture.id} not assigned to graph with ID ${this.id}`
				);
	...
	unassignFromLink(link: LinkController, mirror: boolean = true) {
		if (this._link_ids !== undefined) {
			if (!this._link_ids.includes(link.id))
				throw new Error(
					`GraphError: Link with ID ${link.id} not assigned to graph with ID ${this.id}`
				);
	...
	static revive(cache: ControllerCache, data: SerializedGraph): GraphController {
		...
		if (graph !== undefined) {
			...
			if (!graph.represents(data)) {
				throw new Error(
					`GraphError: Graph with ID ${data.id} already exists, and is inconsistent with new data`
				);
			}

			// Update the existing graph where necessary
			if (graph._course_id === undefined) graph._course_id = data.course_id;
			if (graph._domain_ids === undefined) graph._domain_ids = data.domain_ids;
			if (graph._subject_ids === undefined) graph._subject_ids = data.subject_ids;
			if (graph._lecture_ids === undefined) graph._lecture_ids = data.lecture_ids;
			if (graph._link_ids === undefined) graph._link_ids = data.link_ids;
	...
	represents(data: SerializedGraph): boolean {
		return (
			...
			(this._course_id === undefined ||
				data.course_id === undefined ||
				this._course_id === data.course_id) &&
			(this._domain_ids === undefined ||
				data.domain_ids === undefined ||
				compareArrays(this._domain_ids, data.domain_ids)) &&
			(this._subject_ids === undefined ||
				data.subject_ids === undefined ||
				compareArrays(this._subject_ids, data.subject_ids)) &&
			(this._lecture_ids === undefined ||
				data.lecture_ids === undefined ||
				compareArrays(this._lecture_ids, data.lecture_ids)) &&
			(this._link_ids === undefined ||
				data.link_ids === undefined ||
				compareArrays(this._link_ids, data.link_ids))
		);
	}
	...
	async delete() {
		...
		if (this._course_id !== undefined) this.course.unassignGraph(this);
		if (this._link_ids !== undefined)
			for (const link of this.links) link.unassignGraph(this, false);
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/graph/${this.id} DELETE): ${response.status} ${response.statusText}`
			);
		}
	...
	async reorderDomains(domains?: DomainController[]) {
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/graph/${this.id}/reorder/domains PUT): ${response.status} ${response.statusText}`
			);
		}
	}
	...
	async reorderLectures(lectures?: LectureController[]) {
		...
		if (!response.ok) {
			throw new Error(
				`APIError (/api/graph/${this.id}/reorder/lectures PUT): ${response.status} ${response.statusText}`
			);
		}
	}
	...
	sort() {
		...
		if (this._subjects_relations !== undefined) {
			insertionSort(
				this._subjects_relations,
				(relation) => relation.child?.domain?.order ?? Infinity
			);
			insertionSort(
				this._subjects_relations,
				(relation) => relation.parent?.domain?.order ?? Infinity
			);
		}
	}
	...
	async copy(course: CourseController): Promise<GraphController> {
		// Copy graph
		const copied_graph = await GraphController.create(
			this.cache,
			course,
			`${this.display_name} (Copied from ${this.course.code})`
		);

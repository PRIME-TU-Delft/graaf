...
import { NodeSVG, RelationSVG, OverlaySVG } from '$scripts/svg';
...
class GraphSVG {
	...
	set view(view: EditorView) {
		if (this.view === view || this.state === SVGState.animating) return;
	...
	private set state(state: SVGState) {
		...
		if (this.state === SVGState.broken) {
			d3.select<SVGGElement, unknown>('#overlay').call(OverlaySVG.reset);
		}
	...
	set lecture(lecture: LectureController | null) {
		if (this.lecture === lecture || this.state === SVGState.animating) return;
	...
	get autolayout_enabled() {
		// Check easy case
		if (this.state !== SVGState.dynamic) return false;
		...
		return (
			d3
				...
				.selectAll<
					SVGGElement,
					NodeController<DomainController | SubjectController>
				>('.node:not(.fixed)')
				.size() > 0
		);
	}
	...
	attach(element: SVGSVGElement) {
		...
		this.svg = element;
		const svg = d3
			.select<SVGSVGElement, unknown>(this.svg)
			.attr('display', 'block')
		...
		svg.on('wheel', () => {
			...
			d3.select<SVGGElement, unknown>('#overlay').call(OverlaySVG.shiftWarning, this);
		});
		...
		this.zoom = d3
			...
			.on('zoom', (event) => {
				// Update content
				svg.select('#content').attr('transform', event.transform);
	...
	centerGraph() {
		...
		const bbx = this.boundingBox(
			this.view === 'domains' ? this.graph.domains : this.graph.subjects
		);
	...
	toggleAutolayout() {
		if (this.simulation === undefined) throw new Error('GraphSVG not attached to DOM');
		...
		d3.select('#content')
			...
			.each(async (node) => {
				if (autolayout_enabled) await node.save();
			});
	...
	private validateView(view: EditorView) {
		switch (view) {
			case 'domains':
				return (
					this.graph.domains.every((domain) => domain.validate().severity !== Severity.error) &&
					this.graph.domain_relations.every(
						(relation) => relation.validate().severity !== Severity.error
					)
				);

			case 'subjects':
				return (
					this.graph.subjects.every((subject) => subject.validate().severity !== Severity.error) &&
					this.graph.subject_relations.every(
						(relation) => relation.validate().severity !== Severity.error
					)
				);
			...
			case 'lectures':
				return (
					...
					(this.lecture.validate().severity !== Severity.error &&
						this.lecture.present_subjects.every(
							(subject) => subject.validate().severity !== Severity.error
						))
				);
		}
	}
	...
	private boundingBox(nodes: NodeController<DomainController | SubjectController>[]) {
		if (this.svg === undefined) throw new Error('GraphSVG not attached to DOM');
	...
	private panCamera(x: number, y: number, callback?: () => void) {
		...
		d3.select<SVGSVGElement, unknown>(this.svg)
			...
			.duration(callback !== undefined ? settings.ANIMATION_DURATION : 0)
			.ease(d3.easeSinInOut)
			.call(this.zoom.translateTo, x * settings.GRID_UNIT, y * settings.GRID_UNIT);
	...
	private setBackground(view: EditorView) {
		...
		if (view === 'lectures') {
			...
			const dx =
				(this.svg.clientWidth - 3 * settings.LECTURE_COLUMN_WIDTH * settings.GRID_UNIT) / 2;
			const dy =
			...
			background
				...
				.attr(
					'y',
					dy + settings.STROKE_WIDTH / 2 + settings.LECTURE_HEADER_HEIGHT * settings.GRID_UNIT
				)
			...
			background
				...
				.attr(
					'x',
					dx + (settings.STROKE_WIDTH + settings.LECTURE_COLUMN_WIDTH * settings.GRID_UNIT) / 2
				)
				.attr('y', dy)
				.text('Past Topics')
			...
			background
				...
				.attr(
					'x',
					dx + settings.STROKE_WIDTH / 2 + settings.LECTURE_COLUMN_WIDTH * settings.GRID_UNIT
				)
				.attr(
					'y',
					dy + settings.STROKE_WIDTH / 2 + settings.LECTURE_HEADER_HEIGHT * settings.GRID_UNIT
			...
			background
				...
				.attr(
					'x',
					dx + (settings.STROKE_WIDTH + 3 * settings.LECTURE_COLUMN_WIDTH * settings.GRID_UNIT) / 2
				)
				.attr('y', dy)
				.text('This Lecture')
				.attr('text-anchor', 'middle')
			...
			background
				...
				.attr(
					'x',
					dx + settings.STROKE_WIDTH / 2 + 2 * settings.LECTURE_COLUMN_WIDTH * settings.GRID_UNIT
				)
				.attr(
					'y',
					dy + settings.STROKE_WIDTH / 2 + settings.LECTURE_HEADER_HEIGHT * settings.GRID_UNIT
			...
			background
				...
				.attr(
					'x',
					dx + (settings.STROKE_WIDTH + 5 * settings.LECTURE_COLUMN_WIDTH * settings.GRID_UNIT) / 2
				)
				.attr('y', dy)
				.text('Future Topics')
				.attr('text-anchor', 'middle')
	...
	private clearBackground() {
		d3.select<SVGGElement, unknown>('#background').selectAll('*').remove();
	}

	private setContent(
		nodes: NodeController<DomainController | SubjectController>[],
		relations: RelationController<DomainController | SubjectController>[],
		callback?: () => void
	) {
		if (this.simulation === undefined) throw new Error('GraphSVG not attached to DOM');
		if (nodes.length === 0) {
		...
		content
			...
			.join(
				...
				function (update) {
					return update.call(NodeSVG.updateHighlight, lecture);
				},
		...
		// Update relations
		content
			.selectAll<SVGLineElement, RelationController<DomainController | SubjectController>>(
				'.relation'
			)
			.data(relations, (relation) => relation.uuid)
			.join(
				function (enter) {
					return enter.append('line').call(RelationSVG.create).style('opacity', 0);
				},
		...
		this.simulation.nodes(nodes).force('link', d3.forceLink(links));
	...
	private moveContent(
		nodes: SubjectController[],
		transform: (node: SubjectController) => void,
		callback?: () => void
	) {
	...
	private lectureTransform(x: number, y: number) {
		...
		return (subject: SubjectController) => {
			...
			if (past?.includes(subject)) {
				...
				subject.x =
					dx + settings.STROKE_WIDTH / (2 * settings.GRID_UNIT) + settings.LECTURE_PADDING;
				subject.y =
					dy +
					settings.LECTURE_HEADER_HEIGHT +

...
import { GraphSVG, RelationSVG, SVGState } from '$scripts/svg';
...
class NodeSVG {
	static create(selection: NodeSelection, graphSVG: GraphSVG) {
		...
		selection.selectAll('text').each(function () {
			...
			// Make tspan
			let tspan = element.append('tspan').attr('x', middle_width);
			...
			// Center vertically
			element.selectAll('tspan').attr('y', middle_height - (font_size * line_count * 1.1) / 2);
		});

		// Drag behaviour
		selection.call(
			d3
				.drag<SVGGElement, NodeController<DomainController | SubjectController>>()
	...
	static updateHighlight(selection: NodeSelection, lecture: LectureController | null) {
		selection.each(function (node) {
			const highlight =
				(node instanceof DomainController &&
					lecture?.present_subjects.some((subject) => subject.domain === node)) ||

...
export async function copy(graph_id: number, course_id: number): Promise<SerializedGraph> {
	try {
		let data = await prisma.graph.findUniqueOrThrow({
			where: {
				id: graph_id
			},
		...
		// Create new graph
		var new_graph = await prisma.graph.create({
			data: {
				courseId: course_id,
				name: data.name + ` (copied from ${data.course.code})`
			}
		})

		// Create new domains
		const domain_map = new Map<number, PrismaDomain>() // Map old domain id to newly created domain
		for (const domain of data.domains) {
			const new_domain = await prisma.domain.create({
				data: {
					graphId: new_graph.id,
					name: domain.name,
					style: domain.style,
					order: domain.order,
					x: domain.x,
			...
			domain_map.set(domain.id, new_domain)
		}

		// Create new subjects
		const subject_map = new Map<number, PrismaSubject>() // Map old subject id to newly created subject
		for (const subject of data.subjects) {
			const domain_id = subject.domainId ? domain_map.get(subject.domainId)!.id : null
			const new_subject = await prisma.subject.create({
				data: {
					graphId: new_graph.id,
					domainId: domain_id,
					name: subject.name,
			...
			subject_map.set(subject.id, new_subject)
		}

		// Create new lectures
		const lectures: PrismaLecture[] = [] // Array of newly created lectures
		for (const lecture of data.lectures) {
			const new_lecture = await prisma.lecture.create({
				data: {
					graphId: new_graph.id,
					name: lecture.name,
					order: lecture.order,
		...
		// Connect domains
		for (const [ id, domain ] of domain_map.entries()) {
			const old_domain = data.domains.find(d => d.id === id)
			if (old_domain === undefined) continue // Should never happen

			const new_parents = old_domain.parentDomains.map(parent => domain_map.get(parent.id))
			if (new_parents.some(parent => parent === undefined)) continue // Should never happen
			const delta_parents = prismaUpdateArray<number, PrismaDomain>([], new_parents as PrismaDomain[])
			...
			await prisma.domain.update({
				...
				data: {
					parentDomains: delta_parents
				}
			})
		}

		// Connect subjects
		for (const [ id, subject ] of subject_map.entries()) {
			const old_subject = data.subjects.find(s => s.id === id)
			if (old_subject === undefined) continue // Should never happen

			const new_domain = old_subject.domainId ? domain_map.get(old_subject.domainId) : null
			if (new_domain === undefined) continue // Should never happen
			const delta_domain = prismaUpdateOptionalField<number, PrismaDomain>(null, new_domain)

			const new_parents = old_subject.parentSubjects.map(parent => subject_map.get(parent.id))
			if (new_parents.some(parent => parent === undefined)) continue // Should never happen
			const delta_parents = prismaUpdateArray<number, PrismaSubject>([], new_parents as PrismaSubject[])
		...
		for (const lecture of lectures) {
			const old_lecture = data.lectures.find(l => l.id === lecture.id)
			if (old_lecture === undefined) continue // Should never happen

			const new_subjects = old_lecture.subjects.map(subject => subject_map.get(subject.id))
			if (new_subjects.some(subject => subject === undefined)) continue // Should never happen
			const delta_subjects = prismaUpdateArray<number, PrismaSubject>([], new_subjects as PrismaSubject[])
	...
	return reduce(new_graph, 'course', 'domains', 'subjects', 'lectures', 'links')
}

...
import { GraphHelper, DomainHelper, LectureHelper } from '$scripts/helpers';
...
export async function reduce(
	...
	if (relations.includes('graph')) serialized.graph_id = subject.graphId;
	if (relations.includes('domain')) serialized.domain_id = subject.domainId;
	if (relations.includes('parents'))
		serialized.parent_ids = data.parentSubjects.map((parent) => parent.id);
	if (relations.includes('children'))
		serialized.child_ids = data.childSubjects.map((child) => child.id);
	if (relations.includes('lectures'))
		serialized.lecture_ids = data.lectures.map((lecture) => lecture.id);
...
export async function getAll(...relations: SubjectRelation[]): Promise<SerializedSubject[]> {
	...
	return await Promise.all(subjects.map(async (subject) => await reduce(subject, ...relations)));
}

...
class BackgroundToolbox {
    static init(graph: GraphD3) {
        const pattern = graph.definitions
            .select('pattern#grid')
        if (!pattern.empty()) {
            return
        }
    ...
    static lecture(graph: GraphD3) {
        ...
        const size = graph.lecture ? Math.max(
        ...
        const column_top = settings.STROKE_WIDTH / 2 + settings.LECTURE_HEADER_HEIGHT * settings.GRID_UNIT
        const column_width = (2 * settings.LECTURE_PADDING + settings.NODE_WIDTH) * settings.GRID_UNIT
        const column_height = (size * settings.NODE_HEIGHT + (size + 1) * settings.LECTURE_PADDING) * settings.GRID_UNIT

        const outer_width = 3 * column_width + settings.STROKE_WIDTH
        const outer_height = (size * settings.NODE_HEIGHT + (size + 1) * settings.LECTURE_PADDING + settings.LECTURE_HEADER_HEIGHT) * settings.GRID_UNIT + settings.STROKE_WIDTH

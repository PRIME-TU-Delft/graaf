...
class GraphD3 {
    ...
    get svg() {
        if (this._svg === undefined)
            throw new Error('GraphD3 is detached from dom')
        return this._svg
    }

    private set svg(value) { this._svg = value }
    ...
    private set definitions(value) { this._definitions = value }
    ...
    private set background(value) { this._background = value }
    ...
    private set content(value) { this._content = value }
    ...
    private set overlay(value) { this._overlay = value }
    ...
    private set simulation(value) { this._simulation = value }
    ...
    private set zoom (value) { this._zoom = value }
        ...
        this.svg
            ...
            .on('wheel', () => {
                if (
                    ...
                    this.zoom_lock && !this.keys.Shift
                ) {
    ...
    private moveContent(nodes: NodeData[], transform: (node: NodeData) => void, callback?: () => void) {
    ...
    private lectureTransform(x: number, y: number): (node: NodeData) => void {
        ...
        return (node: NodeData) => {
            ...
            if (past?.includes(node)) {
                ...
                node.y = dy + settings.LECTURE_HEADER_HEIGHT + settings.STROKE_WIDTH / (2 * settings.GRID_UNIT) + (index + 1) * settings.LECTURE_PADDING + index * settings.NODE_HEIGHT
                return
            }
            ...
            if (present?.includes(node)) {
                ...
                node.x = dx + settings.STROKE_WIDTH / (2 * settings.GRID_UNIT) + settings.LECTURE_COLUMN_WIDTH + settings.LECTURE_PADDING
                node.y = dy + settings.LECTURE_HEADER_HEIGHT + settings.STROKE_WIDTH / (2 * settings.GRID_UNIT) + (index + 1) * settings.LECTURE_PADDING + index * settings.NODE_HEIGHT
                return
            }
            ...
            if (future?.includes(node)) {
                ...
                node.x = dx + settings.STROKE_WIDTH / (2 * settings.GRID_UNIT) + 2 * settings.LECTURE_COLUMN_WIDTH + settings.LECTURE_PADDING
                node.y = dy + settings.LECTURE_HEADER_HEIGHT + settings.STROKE_WIDTH / (2 * settings.GRID_UNIT) + (index + 1) * settings.LECTURE_PADDING + index * settings.NODE_HEIGHT
                return
            }
        }

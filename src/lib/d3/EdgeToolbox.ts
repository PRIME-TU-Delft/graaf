...
export class EdgeToolbox {
	...
	static updatePosition(selection: EdgeSelection, transition: boolean = false) {
		...
		selection.each(function (edge) {
			...
			if (Math.abs(dx) < halfWidth && Math.abs(dy) < halfHeight) {
				EdgeToolbox.setLineCoords(
					transition
						? line.transition().duration(settings.GRAPH_ANIMATION_DURATION).ease(d3.easeSinInOut)
						: line,
					cxSource,
					cySource,
					cxTarget,
	...
	/** Set a line's endpoint attributes, in grid units. Works on either a plain selection or a transition. */
	private static setLineCoords<S extends { attr(name: string, value: number): S }>(
		sel: S,
		x1: number,
		y1: number,
		x2: number,
